...
export default function conversionOfReactNativeImport (path: NodePath<t.ImportDeclaration>) {
  ...
  if (isImportFromRN) {
    ...
    if (theReplaceArr.length > 0) {
      const newImport = t.importDeclaration(theReplaceArr , t.stringLiteral('@tarojs/rnap4wx'))
